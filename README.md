...
>**场景：multiple es index based on k8s label**

### create namespce，sa，role
...
### create Fluent Bit daemon set

```
cat << EOF > fluent-bit-daemonset.yaml
---
apiVersion: apps/v1
kind: DaemonSet
metadata:
...
data:
  ...
  output-elasticsearch.conf: |
    [OUTPUT]
        Name            es
        Match           *
        Host            vpc-test-for-fluentbit-with-eks-fn7khfu6oo6vlj7ggkwmoj5wda.us-west-2.es.amazonaws.com
        Port            443
        HTTP_User       jerrywong
        HTTP_Passwd     Luvmi1202!
        TLS             On
        AWS_Auth        Off
        Logstash_Format On
        Logstash_Prefix_Key $kubernetes['labels']['app']
